...
int main(int argc, const char *argv[])
{
	...
	file_out = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0664);
	if (file_out < 0)
	{
		dprintf(STDERR_FILENO, "Error: Can't write to %s\n", argv[2]);
		_close(file_in);
	...
	while ((bytes_read = _read(argv[1], file_in, buffer, BUFSIZE)))
	{
		if (bytes_read < 0)
		{
			_close(file_in);
		...
		if (_write(argv[2], file_out, buffer, bytes_read) < 0)
		{
			_close(file_in);
			_close(file_out);
			exit(99);
	...
	if ((_close(file_in) | _close(file_out)) < 0)
		exit(100);
	return (0);
}
