...
void _close(int gi)
{
	if (close(gi) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't close gi\n", 22);
	exit(98);
}
 ...
 */
void _read(int gi, char *buf, size_t count)
{
	if (read(gi, buf, count) != -1)
		return;
...
int main(int argc, const char *argv[])
{
	...
	lseek(gi, 24, SEEK_SET);
	_read(gi, (char *) buffer, bit_mode / 8);
